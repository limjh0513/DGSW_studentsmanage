...
int Delete(Stu* stu)
{
	...
	stu->korean = NULL; //국어점수 NULL로 초기화
	stu->data_str = NULL; //자료구조 점수 NULL로 초기화
}
...
int DGSW_Check(int grade)
{
	...
	i = grade / 1000; //학번 받은 부분에서 학년만 추출
	j = grade / 100 % 10; //학번 받은 부분에서 반만 추출
	k = grade % 100; //학번 받은 부분에서 번호만 추출
...
void scorePrint()
{
	printf("--------------------------------------------------\n\n");
	printf("1번 : 국어\n2번 : 자료구조\n");
	printf("(가장 높은 점수를 가진 학생이 2명이라면\n다른 과목의 점수가 더 높은 학생을 출력합니다.)\n\n");
	printf("--------------------------------------------------\n");
	printf("입력 : ");
}